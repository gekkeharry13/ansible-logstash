...
shared_examples 'xpack::init' do |vars|
  ...
  describe 'logstash4_server access check' do
    it 'should be reported as version '+vars['es_version'] do
      command = command('curl -s localhost:9200/ -u logstash4_server:changeMe | grep number')
      expect(command.stdout).to match(vars['es_version'])
      expect(command.exit_status).to eq(0)
    end
  end

  describe command('curl -s localhost:9200/_xpack/security/user/logstash4_server -u elastic:elasticChanged | md5sum | grep f0548742161d9e50c7c7fbe2e061a1fa') do
      its(:exit_status) { should eq 0 }
  end
  ...
  if vars['es_major_version'] == '5.x' # logstash default password has been removed in 6.x
    describe 'logstash access check' do
      it 'should be reported as version '+vars['es_version'] do
        result = curl_json('http://localhost:9200/', username='logstash', password='changeme')
        expect(result['version']['number']).to eq(vars['es_version'])
      end
    end
  end

  describe command('curl -s localhost:9200/_xpack/security/user/logstash -u elastic:elasticChanged | md5sum | grep 34190c64eb3c7cfb002fa789df5fad20') do
      its(:exit_status) { should eq 0 }
  end
